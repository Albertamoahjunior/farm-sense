...
\begin{figure}[h]
\includegraphics[scale=0.5]{{/final-architecture.jpg}}
\caption{System Architecture}
\end{figure}
In the above image is the block diagram of the system architecture of the farm monitoring system. The system is made up of five(5) major blocks. The Sensor node, The power supply unit, the cloud service been used and the client interface. All these separate parts come together to monitor, transmit and analyse the vital farm conditions in scope. The sensor node monitors the various factors in scope, the management node is collects data from the sensor node and sends them to the cloud service from which information is sent to the clients end.  
\newpage
