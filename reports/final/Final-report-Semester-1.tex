...
\end{figure}
In the above image is the block diagram of the system architecture of the farm monitoring system. The system is comprised of five (5) major blocks: the Sensor node, the power supply unit, the cloud service being used, and the client interface. All these separate parts come together to monitor, transmit, and analyze the vital farm conditions in scope.\\

The sensor node monitors various factors in scope, while the management node collects data from the sensor node and sends it to the cloud service, from which information is then sent to the client's end.
\newpage
\subsubsection{Sensor Node}
\begin{figure}[h]
\includegraphics[scale=0.5]{{/Sensor-node.jpg}}
\caption{Sensor Node}
\end{figure}
...
The central unit, or the intelligent unit, gathers information from all the sensor modules and then transmits it over WiFi to the management node in the system.\\
...
The system is built such that for every four or fewer sensor nodes, there is a management node set to manage them. The management node is in charge of receiving data from the sensor nodes, refining the data, and sending it to the cloud service.

\subsubsection{Cloud Services}
The cloud services are in charge of the data analysis of the farm conditions data sent via the internet to them. Upon the analysis made by the cloud services, recommendations will be formulated for farmers to follow to achieve high crop yield and avoid wasting resources. The cloud service also serves as the backend for the mobile app user interface. Additionally, the cloud service takes care of sending SMS messages to users using SMS APIs.

\subsubsection{Power Supply}
The power supply unit consists of a battery and a solar panel. The solar panel charges the battery, and the power from the battery is utilized to power the sensor or management node. Each node has its own power supply unit, which is responsible for meeting the power needs of its components.

\subsubsection{Client Interface}
All the information gathered by the sensors and analyzed by the cloud services will go to waste if there is no way of presenting the information derived from the analysis and the recommendations from the cloud services.\\

The client interface consists of two parts. Considering our niche/market in Ghana and other African countries, an SMS feature was added. Information about the farm conditions and recommendations are sent through SMS. Additionally, there is the Graphic User Interface (GUI) which provides more detailed information about the farm condition and recommends habits that will increase crop yield based on accurate data.
\newpage
